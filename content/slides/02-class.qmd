...
1.  Using the RStudio console, find the summation of `45`, `978`, and `121`.
2.  What is `67` divided by `6`?
3.  What is the square root of `894`? HINT: use the `sqrt()` function.
...
1.  Using the RStudio console, install the `tidyverse` packages.
...
2.  Load these packages in your current session
...
3.  Re-run `getwd()` to see where you now are on your computer.
...
1.  Take a closer look at the `hwy` and `cyl` variables. Describe them to me.

2.  Make a scatter plot of them.

## EXERCISE {background-color="#B56576"}

3.  Take a closer look at the `class` and `drv` variables. Describe them to me.

4.  Make a scatter plot of them. Why doesn't this work as a data visualization?
...
2.  Map `class` to the `shape` aesthetic. What does the warning tell you?
